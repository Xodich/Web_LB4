...
_toConsumableArray(popoverTriggerList).map(function (popoverTriggerEl) {
  return new bootstrap__WEBPACK_IMPORTED_MODULE_1__.Popover(popoverTriggerEl);
});

// Переключение по стрелкам
var infoModal = document.getElementById('infoModal');
var detailButtons = document.querySelectorAll('.item-card .btn-primary');
var currentButtonIndex = -1;
if (infoModal) {
  var modalInstance = new bootstrap__WEBPACK_IMPORTED_MODULE_1__.Modal(infoModal);
  infoModal.addEventListener('show.bs.modal', function (event) {
    var button = event.relatedTarget;
    currentButtonIndex = Array.from(detailButtons).findIndex(function (btn) {
      return btn === button;
    });
    var modalTitle = button.getAttribute('data-modal-title');
    var modalBodyRaw = button.getAttribute('data-modal-body');
    var modalImgSrc = button.getAttribute('data-modal-img');
    var modalAbilitiesRaw = button.getAttribute('data-abilities');
    var modalBodyHtml = modalBodyRaw.replace(/\\n/g, '<br><br>');
    ...
    infoModal.querySelector('#modal-content-img-container').innerHTML = "\n            <img src=\"".concat(modalImgSrc, "\" alt=\"").concat(modalTitle, "\" class=\"img-fluid rounded-3\" style=\"max-height: 200px; object-fit: cover;\">\n        ");
    var abilitiesContainer = infoModal.querySelector('#modal-abilities-container');
    abilitiesContainer.innerHTML = '';
    if (modalAbilitiesRaw) {
      var abilitiesHtml = '<h5 class="mb-3">Способности</h5>';
      modalAbilitiesRaw.split(';').forEach(function (ability) {
        ...
        if (parts.length === 3) {
          ...
          abilitiesHtml += "\n                        <div class=\"ability-item mb-3 d-flex align-items-start\">\n                            <img src=\"".concat(iconPath, "\" alt=\"").concat(title, "\" class=\"ability-icon me-3\">\n                            <div>\n                                <h6 class=\"ability-title mb-1\">").concat(title, "</h6>\n                                <p class=\"ability-description mb-0\">").concat(description, "</p>\n                            </div>\n                        </div>\n                    ");
        }
      });
  ...
  document.addEventListener('keydown', function (event) {
    if (infoModal.classList.contains('show')) {
      ...
      if (targetButtonIndex !== -1) {
        event.preventDefault();
        modalInstance.hide();
        var _hiddenHandler = function hiddenHandler() {
          detailButtons[targetButtonIndex].click();
          infoModal.removeEventListener('hidden.bs.modal', _hiddenHandler);
        };
        infoModal.addEventListener('hidden.bs.modal', _hiddenHandler);
      }
    }
  });
}
...
if (toastTrigger && toastLiveExample) {
  var toastBootstrap = new bootstrap__WEBPACK_IMPORTED_MODULE_1__.Toast(toastLiveExample);
